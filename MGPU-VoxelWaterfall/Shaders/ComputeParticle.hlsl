...
float3 DeterministicSpawnPosition(uint voxelIndex)
{
    ...
    const uint horizontalIndex = (voxelIndex + EmitterBuffer.Seed) % horizontalCells;
    const uint xIndex = horizontalIndex % widthCells;
    const uint zIndex = horizontalIndex / widthCells;
    const uint topLayer = (voxelIndex / horizontalCells) % 4u;
...
void CS(uint3 groupID : SV_GroupID, uint groupIndex : SV_GroupIndex)
{
    ...
    if (particle.ContinuousPosition.y <= EmitterBuffer.FloorHeight)
    {
        ...
        particle.Velocity = float3(0.0f, -EmitterBuffer.InitialFallSpeed, 0.0f);
    }
